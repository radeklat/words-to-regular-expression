...
class IterableToRegexp(TestCase):
    ...
    def test_generates_correct_regexp_from_iterable_of_strings(self, expected_strings):
        assert_strings_can_be_matched(
            self, iterable_to_regexp(expected_strings, PythonFormater), expected_strings
        )

    def test_it_ignores_empty_strings(self):
        self.assertEqual(
            PythonFormater._EMPTY_STRING_MATCH, iterable_to_regexp([''], PythonFormater)
        )

    def test_it_defaults_to_python_formater(self):
        self.assertEqual(
            PythonFormater._EMPTY_STRING_MATCH, iterable_to_regexp([''])
        )
